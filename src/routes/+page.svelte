...
{#if message}
  <section class="flex flex-col space-y-4 justify-start items-start p-4 bg-blue-50 border border-blue-200 text-blue-900 rounded-md my-4">
    <p>{message}</p>
    <button class="bg-blue-100 px-2 py-1 rounded text-blue-900" on:click={clearMessage}>Close</button>
  </section>
{/if}
          ...
          {#if entry.categories}
            <ul class="flex space-x-1 pt-1">
              {#each entry.categories as category}
                <li>
                  <small class="bg-purple-100 text-purple-700 border border-purple-300 px-1 rounded-md block">{category.category.name}</small>
                </li>
              {/each}
            </ul>
          {/if}
